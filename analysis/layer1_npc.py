...
Input:  data/npc_scores.jsonl.gz
Output: printed tables (stdout) + output/layer1_npc_results.json

Usage:
    python analysis/layer1_npc.py
    python analysis/layer1_npc.py --data path/to/npc_scores.jsonl.gz
"""
...
def parse_args():
    ...
    p.add_argument(
        "--data",
        default="data/npc_scores.jsonl.gz",
        help="Path to npc_scores.jsonl.gz",
    )
    p.add_argument(
        "--out", default="output/layer1_npc_results.json", help="Output JSON path"
    )
    return p.parse_args()
...
def ols_length_adjusted_d(art_vals, doaj_vals, art_log_nouns, doaj_log_nouns):
    """
    OLS: metric ~ intercept + is_art + log1p(nouns)
    Returns length-adjusted Cohen's d matching robustness_length_regression.py:
      d = beta[is_art] / pooled_sd
    where pooled_sd = sqrt((sd_art² + sd_doaj²) / 2).
    """
    # Build design matrix [1, is_art, log_nouns]
    # is_art=1 for art, is_art=0 for DOAJ (matching paper's src convention)
    X = [(1, 1, ln) for ln in art_log_nouns] + [(1, 0, ln) for ln in doaj_log_nouns]
    ...
    # Pooled SD: simple average of group variances (matching paper's convention)
    art_sd = statistics.stdev(art_vals) if len(art_vals) > 1 else 1.0
    doaj_sd = statistics.stdev(doaj_vals) if len(doaj_vals) > 1 else 1.0
    pool_sd = math.sqrt((art_sd**2 + doaj_sd**2) / 2)

    # beta[1] is the Art dummy coefficient (positive when Art > DOAJ)
    adj_d = beta[1] / pool_sd if pool_sd > 0 else 0.0
    return adj_d, beta
...
def main():
    ...
    with opener(data_path, "rt", encoding="utf-8") as f:
        for line in f:
            ...
            if pre is None or post is None or nn < 5:
                continue
            ln = math.log1p(nn)  # log(n+1), matching robustness_length_regression.py
            if d["institution"] == "doaj":
                doaj_pre.append(pre)
                doaj_post.append(post)
                doaj_ln.append(ln)
            else:
                art_pre.append(pre)
                art_post.append(post)
                art_ln.append(ln)
    ...
    for label, art_vals, doaj_vals, art_log_n, doaj_log_n in [
        ...
        z90 = 1.645
        se = math.sqrt(
            (len(art_vals) + len(doaj_vals)) / (len(art_vals) * len(doaj_vals))
        ...
        print(
            f"  Length-adjusted d: {adj_d:.3f}  [95% CI: {adj_ci_lo:.3f}, {adj_ci_hi:.3f}]"
        )
        print()
        ...
        results[label] = {
            ...
            "doaj_mean": doaj_mean,
            "doaj_sd": doaj_sd,
            "doaj_n": len(doaj_vals),
            "cohen_d": d_val,
            "ci_95": [ci_lo, ci_hi],
            "tost_90ci": [ci90_lo, ci90_hi],
            "tost": tost,
            "length_adjusted_d": adj_d,
            "length_adjusted_d_ci95": [adj_ci_lo, adj_ci_hi],
        }
